Hi, I'm Shubhendu Vimal 👋

Principal Product Manager | Agentic AI & Enterprise SaaS ERP | Bengaluru

I ship AI products that work — not just demos. 11+ years shipping ML models, multi-agent agentic systems, and self-serve analytics for Fortune 500 banking clients and mid-market finance teams. Built products at Oracle NetSuite (78% ARR growth, $1.6M monetization opportunities identified) and Fractal Analytics (CPG revenue optimization platform).
Currently in stealth mode on next-generation agentic PM work — demonstrating both execution rigor and deep technical AI fluency.

### 🚀 What I've built

**[AskERP](https://github.com/Shubhendu123/askerp)** — Multi-agent LLM → SQL → ERP analytics. Shipped to Oracle NetSuite customers. Semantic routing, schema awareness, agentic reasoning.

**[Counsel AI](https://github.com/Shubhendu123/Counsel-AI)** — Agentic RAG for legal analysis. Multi-agent retrieval + reasoning pipeline. Trust Console for auditability. Production-grade reliability patterns.

**[Portfolio Site](https://shubhendu-portfolio-eight.vercel.app)** — PM case studies (Oracle, Fractal) +live AI demos. 

Banking client work: Cash forecasting (predictive ML), churn prediction (segmentation + intervention), dynamic pricing (markdown optimization), NLG narratives (automated insights).

🧠 Credentials

**Education**
- PhD in Finance, IFTM University
- Advanced Management Programme in Business Analytics, ISB Hyderabad — Scholar of Excellence
- MBA, Amity University
- MA Economics, MJPRU
- B.Tech, Dr. A.P.J. Abdul Kalam Technical University
- Tech stack: Python · SQL · Claude/OpenAI APIs · RAG · Multi-agent systems · Next.js · Azure/GCP/AWS/OCI
PM depth: Product strategy, monetization, technical roadmaps, customer adoption metrics

### 📬 Let's connect

- **[LinkedIn](https://www.linkedin.com/in/shubhendu-vimal)**
- **[Portfolio](https://shubhendu-portfolio-eight.vercel.app)**
- **[GitHub](https://github.com/Shubhendu123)**
